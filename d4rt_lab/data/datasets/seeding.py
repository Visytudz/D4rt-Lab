...
def _dataset_children(dataset: object) -> list[object]:
...
def configure_dataset_seeding(dataset: object, base_seed: int) -> None:
    configure = getattr(dataset, "configure_dataset_seed", None)
    if callable(configure):
        configure(int(base_seed))
    for child in _dataset_children(dataset):
        configure_dataset_seeding(child, base_seed)


def set_dataset_epoch_recursive(dataset: object, epoch: int) -> None:
    set_epoch = getattr(dataset, "set_dataset_epoch", None)
...
def set_dataset_worker_recursive(dataset: object, worker_id: int) -> None:
    set_worker = getattr(dataset, "set_dataset_worker", None)
    if callable(set_worker):
        set_worker(int(worker_id))
    for child in _dataset_children(dataset):
