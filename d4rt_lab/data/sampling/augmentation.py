"""Raw-data augmentation and hard-boundary helpers."""
...
from d4rt_lab.data.config import AugmentationConfig
...
CropInfo = dict[str, tuple[int, int]]
...
def build_augment_info(
    ...
    *,
    ...
    """Build the fixed-shape augmentation metadata stored with each sample."""
    out_h, out_w = image_hw
    info = crop_info or {}
    crop_h, crop_w = info.get("crop_hw", (out_h, out_w))
    crop_x, crop_y = info.get("crop_xy", (0, 0))
    return {
        "crop_hw": np.asarray([crop_h, crop_w], dtype=np.int32),
        "crop_xy": np.asarray([crop_x, crop_y], dtype=np.int32),
        "crop_applied": np.asarray(["crop_hw" in info], dtype=np.bool_),
    }


def _probability_if_enabled(enabled: bool, probability: float) -> float:
    return float(probability) if enabled else 0.0


def _normalize_supported_mode(value: str, *, path: str, supported: set[str]) -> str:
...
def augment_cfg_from_train_config(cfg: AugmentationConfig) -> RawAugmentConfig:
    """Convert structured augmentation settings to dataset augmentation settings."""
    crop = cfg.random_crop
    temporal = cfg.temporal_subsample
    jitter = cfg.color_jitter
    color_drop = cfg.color_drop
    ...
    stride_sampling = _normalize_supported_mode(
        temporal.stride_sampling,
        path="augmentation.temporal_subsample.stride_sampling",
        supported={"random"},
    )

    return RawAugmentConfig(
        color_jitter_enabled=jitter.enabled,
        jitter_brightness_enabled=jitter.brightness > 0,
        jitter_saturation_enabled=jitter.saturation > 0,
        jitter_contrast_enabled=jitter.contrast > 0,
        jitter_hue_enabled=jitter.hue > 0,
        color_drop_prob=_probability_if_enabled(color_drop.enabled, color_drop.probability),
        blur_prob=_probability_if_enabled(blur.enabled, blur.probability),
        random_crop_enabled=crop.enabled,
        ...
        random_zoom_prob=float(crop.random_zoom_in_probability),
        temporal_subsample_enabled=temporal.enabled,
        temporal_subsample_stride_sampling=stride_sampling,
        temporal_subsample_stride_min=int(temporal.stride_min),
        temporal_subsample_stride_max=int(temporal.stride_max),
    )
